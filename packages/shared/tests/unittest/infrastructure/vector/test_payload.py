...
class TestVectorPayload(unittest.TestCase):
    def test_payload_text_prefers_direct_text_field(self) -> None:
        # Arrange
        payload = {"text": "  hello world  ", "_node_content": '{"text":"ignored"}'}
    ...
    def test_payload_text_parses_node_content_json(self) -> None:
        ...
        # Act / Assert
        self.assertEqual(payload_text({"_node_content": node}), "from node")
    ...
    def test_payload_text_returns_raw_when_json_invalid(self) -> None:
        ...
        # Act / Assert
        self.assertEqual(payload_text({"_node_content": raw}), "plain fallback")

    def test_payload_text_empty_payload(self) -> None:
        # Act / Assert
        self.assertEqual(payload_text({}), "")
        self.assertEqual(payload_text({"_node_content": ""}), "")

    def test_payload_page_prefers_page_over_page_label(self) -> None:
        # Act / Assert
        self.assertEqual(payload_page({"page": 3, "page_label": "III"}), 3)

    def test_payload_page_falls_back_to_page_label(self) -> None:
        # Act / Assert
        self.assertEqual(payload_page({"page_label": "12"}), "12")

    def test_payload_page_none_when_missing(self) -> None:
        # Act / Assert
        self.assertIsNone(payload_page({}))
