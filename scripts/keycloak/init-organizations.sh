...
# Applies declarative user profile, then seeds demo/test users via add-user.sh.
set -eu

SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
REALM_DIR="${REALM_DIR:-${SCRIPT_DIR}/../../operations/keycloak/realm}"
...
echo "Ensuring organizations tenant-a, tenant-b..."
create_org "Tenant A" "tenant-a" >/dev/null
create_org "Tenant B" "tenant-b" >/dev/null

make -C "${WORKSPACE}" add-users
