...
import { ACTION_TIMEOUT_MS } from "./timeouts";

function humanMode(): boolean {
  return process.env.DEMO_HUMAN === "1";
}

function mouseSteps(): number {
  return humanMode() ? Number(process.env.DEMO_MOUSE_STEPS ?? 28) : 1;
}

function pauseMs(): number {
  return humanMode() ? Number(process.env.DEMO_PAUSE_MS ?? 400) : 0;
}

function typingDelayMs(): number {
  return humanMode() ? Number(process.env.DEMO_TYPING_DELAY_MS ?? 45) : 0;
}
...
export async function humanMoveTo(page: Page, target: Locator): Promise<void> {
  await target.waitFor({ state: "visible", timeout: ACTION_TIMEOUT_MS });
  await target.scrollIntoViewIfNeeded();
  const box = await target.boundingBox();
  if (!box) return;
  await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2, { steps: mouseSteps() });
}
